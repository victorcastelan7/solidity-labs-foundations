...
contract TestErrors is Test {
  ...
  function testWithdrawRevertsIfInsufficientBalance() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        Errors.Errors__InsufficientBalance.selector,
        withdrawAmount,
        balance)
      );
  ...
  function testWithdrawUpdatesMapping() public {
    uint256 withdrawAmount = .02 ether;
  ...
  function testSweepTransfersFunds() public {
    ...
    assertEq(contractBalanceAfterDeposit, (amountOfUsers - 1) * depositAmount, "Contract balance should have received all deposits");
    ...
    assertEq(contractBalanceAfterDeposit, recipientBalanceAfterSweep, "Recipient should receive full contract balance");
